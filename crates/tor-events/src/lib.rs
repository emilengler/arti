...
pub fn broadcast(event: TorEvent) {
    eprintln!("broadcast {:?}", event);
    if !event_has_subscribers(event.kind()) {
        eprintln!("no subscribers");
        return;
    }
    if let Some(sender) = EVENT_SENDER.get() {
        // If this fails, there isn't much we can really do about it!
        let ret = sender.unbounded_send(event);
        if ret.is_err() {
            eprintln!("event broadcast failed due to closed channel");
        } else {
            eprintln!("unbounded sent");
        }
    } else {
        eprintln!("no event sender");
    }
}
