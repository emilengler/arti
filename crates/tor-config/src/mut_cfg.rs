...
///
/// Internally, this is just a `Mutex<Arc<T>>`; this type just defines some
...
impl<T> MutCfg<T> {
    ...
    pub fn new(config: T) -> Self {
        Self {
            cfg: Mutex::new(Arc::new(config)),
        }
    }

    /// Return the current configuration
    pub fn get(&self) -> Arc<T> {
        Arc::clone(&self.cfg.lock().expect("poisoned lock"))
    }
    ...
    pub fn check_and_replace(&self, old_config: &Arc<T>, new_config: T) -> bool {
        let mut cfg = self.cfg.lock().expect("poisoned lock");
    ...
    pub fn replace(&self, new_config: T) {
        *self.cfg.lock().expect("poisoned lock") = Arc::new(new_config);
    }
}
...
impl<T: Default> Default for MutCfg<T> {
    fn default() -> Self {
        MutCfg {
            cfg: Mutex::new(Arc::new(T::default())),
        }
    }
}
